...
const toggleLike = (bookId: number) => {
  ...
  if (book) {
    ...
    book.likeCount += book.isLiked ? 1 : -1
  }
}
              ...
              <button
                ...
                :class="
                  css({
                    ...
                    px: '3',
                    py: '2.5',
                    bg: book.isLiked ? 'red.50' : 'gray.100',
                    color: book.isLiked ? 'red.500' : 'gray.600',
                    border: '1px solid',
                    borderColor: book.isLiked ? 'red.200' : 'gray.200',
                    borderRadius: '0.5rem',
                    fontSize: '0.875rem',
                    fontWeight: '500',
                    cursor: 'pointer',
                    transition: 'all 0.2s',
                    _hover: {
                      bg: book.isLiked ? 'red.100' : 'gray.200',
                      borderColor: book.isLiked ? 'red.300' : 'gray.300',
                    },
                  })
                "
                @click.stop="toggleLike(book.id)"
              >
                <i :class="book.isLiked ? 'pi pi-heart-fill' : 'pi pi-heart'" />

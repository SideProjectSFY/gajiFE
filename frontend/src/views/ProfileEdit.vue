...
onMounted(() => {
  ...
  form.bio = (profile.value as any).bio || ''
})
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (authStore.user) {
      ...
      ;(authStore.user as any).bio = response.data.bio
      if (response.data.avatarUrl) {
        (authStore.user as any).avatarUrl = response.data.avatarUrl
      }
    }
    ...
    errors.general = error.response?.data?.message || 'Failed to update profile'
  } finally {

...
const router = createRouter({
  ...
  routes: [
    ...
    { path: '/books/:bookId/scenarios/:scenarioId', component: { template: '<div>Scenario</div>' } },
  ],
})
...
describe('ScenarioCreationModal', () => {
  ...
  describe('validation', () => {
    ...
    it('enables submit button when title + valid scenario type', async () => {
      const wrapper = mountModal()
      await wrapper.find('#scenario-title').setValue('Test Title')
      await wrapper.find('#character-changes').setValue('This is a valid character change description')
      await flushPromises()
    ...
    it('shows validation error when typing invalid content', async () => {
      ...
      expect(wrapper.text()).toContain('Please provide at least one scenario type with 10+ characters')
    })
  ...
  describe('form submission', () => {
    it('calls API with correct payload on submit', async () => {
      ...
      await wrapper.find('#character-changes').setValue('Hermione sorted into Slytherin instead of Gryffindor')
